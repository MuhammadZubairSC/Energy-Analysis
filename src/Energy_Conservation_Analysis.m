...
%
%Plot Refreshing 
cla(handles.Hilly_Terrain_Plot); 
cla(handles.Kinetic_Potential_Energy_Plot); 
%
...
%
if (isempty(vehicle_one)|| vehicle_one==1) 
    msgbox('Simulation requires the user to select the vehicle from Vehicle garage','Error Message','custom',imread('Vehicleselection.png'));
...
if (isempty(track))  % if condition executes there is no value in the track variable
    msgbox('Simulation requires the user to select from the options given in Track Outlook Panel','Error Message','custom',imread('TrackSelection.png')); 
    return;% exits from the function
end % end of if construct
%
%Error Handling for Elevation Panel
%
if ((isempty(h0))| (isempty(h1))|(isempty(h2))| (isempty(h3))|(isempty(h4))|(isempty(h5))|(isempty(h6))|(isempty(h7))|(isempty(h8))|(isempty(h9))|(isempty(h10)))
    msgbox('Please enter some authentic value for height.','Error Message');
    return;
end 
%
...
if ((isempty(v0))|(isempty(v1))|(isempty(v2)))
     msgbox('Please enter some authentic value for  speed.','Error Message');
    return;
end 
...
axes(handles.Hilly_Terrain_Plot);
set(handles.Hilly_Terrain_Plot,'Color',[0 0.9 0.9]);
hold on; 
switch (vehicle_one)
    case 1 % if the value of track is one
